# Error in run_04: Files.upload() got an unexpected keyword argument 'path'
import os
import time
import pandas as pd
...
class MagicAnalyzer:
    def __init__(self):
        try:
            ...
            # High-performance model identified during discovery
    ...
    def run_inference(self, video_path, data_summary, context):
        ...
        # Uploading to Google File API to get a valid URI
        ...
        # Construct multimodal contents using the File API URI
        contents = [
            types.Part.from_uri(
...
""")
        ...
        # Cleanup: delete remote file after inference
...
def main():
    ...
    for run_id in run_folders:
        try:
            ...
            # Handle nested directory structure like data/run_03/run_03/video.mov
            ...
            # Append mode for real-time saving
            with open(OUTPUT_FILE, "a", encoding="utf-8") as f:
                f.write(f"## Analysis: {run_id}\n\n{result}\n\n---\n\n")

            print("Success. Cooling down 45s for quota protection...")
            time.sleep(45)
